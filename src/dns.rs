...
//! записи от иностранных резолверов, просто перестают открываться; на роутере
//! этот же сплит стоит по той же причине. Но и он спрашивается шифрованно:
...
///
/// Спрашиваем его тоже по DoH, а не открытым UDP на 53. С августа 2026 ТСПУ
/// перехватывает обычный DNS на 53 и отвечает за чужой резолвер — открытый
/// путь перестал быть просто «нешифрованным», он стал недостоверным.
/// Сертификат Яндекса выписан прямо на адреса (`IP Address:77.88.8.8` в SAN),
/// поэтому DoH идёт по IP и в бутстрапе не нуждается. Российским резолвер
/// при этом быть не перестал: наружу мы выходим со своего адреса, и банки
/// по-прежнему отдают нам свои российские записи.
...
/// Имя-метка, по которой узнаётся роутер этого же кита: его dnsmasq отвечает
/// на неё фиксированным адресом, а больше никто в мире не отвечает вовсе.
///
/// Домен в зоне `.invalid` (RFC 2606) существовать не может по определению,
/// поэтому положительный ответ — уже сам по себе признак. Адрес из
/// документационной сети 192.0.2.0/24 (RFC 5737): защита от DNS-rebind на
/// роутере режет ответы в 127.0.0.0/8 и в частных сетях, и метка на таком
/// адресе просто не доезжала.
pub const МЕТКА: &str = "kit.netpult.invalid";
const МЕТКА_ОТВЕТ: [u8; 4] = [192, 0, 2, 7];

/// Стоит ли за нами роутер, который сам шифрует DNS и сам раскладывает домены
/// по маршрутам.
///
/// Это не украшение отчёта, а защита от тихой поломки. Роутер кита решает,
/// какой сайт идёт через ноду, по адресам, которые видит его же dnsmasq: имя
/// спросили — адрес попал в набор — трафик на этот адрес свернул в туннель.
/// Свой резолвер на машине обходит dnsmasq стороной, роутер нужных адресов не
/// видит, и сайт уходит напрямую. Внешне это выглядит как «саундклауд не
/// грузится без VPN», а не как поломка DNS, и потому ищется долго.
pub fn шлюз_кита() -> Option<String> {
    let gw = crate::route::default_exit()?.gateway?;
    let ответ = ask(&gw, 53, МЕТКА, Duration::from_secs(2))?;
    ответ
        .адреса
        .iter()
        .any(|a| a.octets() == МЕТКА_ОТВЕТ)
        .then_some(gw)
}
...
pub fn on(cfg: &Config) -> Result<Vec<String>, String> {
    ...
    // За роутером кита свой резолвер не нужен и прямо вреден: он уводит
    // запросы мимо роутерного dnsmasq, а тот по ним раскладывает домены по
    // маршрутам. Поэтому вместо «поднять своё» здесь правильный ответ —
    // отдать DNS роутеру.
    if let Some(gw) = шлюз_кита() {
        // Снять свою службу и вернуть системе её DNS — обе операции идут
        // через sudo, поэтому пароль спрашиваем до, а не посреди работы.
        if !cfg!(windows) {
            crate::sudoer::ready()?;
        }
        ...
        if служба_поднята() {
            снять_службу()?;
            шаги.push("свой резолвер снят".into());
        }
        отвязать_систему()?;
        шаги.push(format!("DNS отдан роутеру {gw}"));
        шаги.push("шифрует и раскладывает домены он сам".into());
        return Ok(шаги);
    }
...
mod tests {
    ...
    fn в_конфиге_есть_и_доh_и_российский_резолвер() {
        ...
        // Оба канала шифрованные. Открытый UDP тут был бы не «чуть хуже», а
        // недостоверным: на 53 отвечает ТСПУ, а не тот, кого спросили.
